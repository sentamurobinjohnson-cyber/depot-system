...
const titles = {
  dashboard:['Dashboard','Today’s business summary'], items:['Items / Stock','Create items and manage prices'], receive:['Received Items','Add stock from suppliers'], sales:['Sales','Sell items and calculate profit'], expenses:['Expenses','Track depot costs'], customers:['Customers','Manage customer records'], suppliers:['Suppliers','Manage supplier records'], reports:['Reports','Profit / loss and daily totals'], backup:['Backup','Export, restore, or reset data']
};
...
function render(){ options(); renderItems(); renderReceive(); renderSales(); renderExpenses(); renderPeople(); renderDashboard(); renderReports(); }

function renderItems(){
  const q = ($('itemSearch')?.value || '').toLowerCase();
  $('itemsTable').innerHTML = db.items.filter(i=>i.name.toLowerCase().includes(q)).map(i=>{
    const profit = i.retail - i.cost; const margin = i.retail ? (profit / i.retail * 100) : 0;
    return `<tr><td>${i.name}</td><td>${i.qty}</td><td>${i.unit}</td><td>${money(i.cost)}</td><td>${money(i.retail)}</td><td>${money(profit)}</td><td>${margin.toFixed(1)}%</td><td>${actionBtn('Edit',`editItem('${i.id}')`)} ${actionBtn('Delete',`deleteItem('${i.id}')`,'danger')}</td></tr>`;
...
function renderSales(){
  $('salesTable').innerHTML = db.sales.slice().reverse().map(s=>{
    ...
    return `<tr><td>${s.date}</td><td>${c.name||''}</td><td>${item.name||'Deleted item'}</td><td>${s.qty}</td><td>${money(sales)}</td><td>${money(cost)}</td><td class="${profit>=0?'profit':'loss'}">${money(profit)}</td><td>${actionBtn('Delete',`deleteSale('${s.id}')`,'danger')}</td></tr>`;
...
$('saleItem').onchange=()=>{ const i=byId(db.items,val('saleItem')); if(i) $('salePrice').value=i.retail; };
$('saleForm').onsubmit=e=>{ e.preventDefault(); const item=byId(db.items,val('saleItem')); if(!item) return toast('Create an item first'); const qty=num('saleQty'); if(qty>item.qty) return toast('Not enough stock'); const price=num('salePrice'); db.sales.push({id:uid(),date:val('saleDate'),customerId:val('saleCustomer'),itemId:item.id,qty,price,cost:item.cost,payment:val('salePayment')}); item.qty-=qty; save(); e.target.reset(); setDates(); render(); toast('Sale saved'); };
...
function seed(){
  db={ items:[{id:'i1',name:'Cement',unit:'bag',qty:100,cost:30000,retail:37000,alert:10},{id:'i2',name:'Sugar',unit:'kg',qty:50,cost:3500,retail:4500,alert:8},{id:'i3',name:'Cooking Oil',unit:'litre',qty:30,cost:6500,retail:8000,alert:5}], customers:[{id:'walkin',name:'Walk-in Customer',phone:'',address:''},{id:'c1',name:'Mukasa Shop',phone:'0700000000',address:'Kampala'}], suppliers:[{id:'unknown',name:'Unknown Supplier',phone:'',address:''},{id:'s1',name:'Depot Wholesalers',phone:'0750000000',address:'Industrial Area'}], received:[], sales:[], expenses:[] };
  db.received.push({id:'r1',date:today(),supplierId:'s1',itemId:'i1',qty:20,unitCost:30000,notes:''});
  db.sales.push({id:'sl1',date:today(),customerId:'walkin',itemId:'i1',qty:5,price:37000,cost:30000,payment:'Cash'});
